...
if(supabaseUrl && supabaseKey){
  ...
  supabase = { from: ()=>({ select: async()=>({data:[],error:{message:"Supabase not configured"}}) }) };
}
...
function loadData() {
  try {
    ...
    cachedData.forEach(r => Object.keys(r).forEach(k => headerSet.add(k)));
    headerOrder = Array.from(headerSet);
    console.log(`Loaded data.json: ${cachedData.length} records`);
  } catch(e){ cachedData=[]; headerOrder=[]; console.error(e);}
}
function loadUsers() {
  try { usersData = require('./users.json'); console.log(`Loaded users.json: ${usersData.length} users`); }
  catch(e){ usersData=[]; console.error(e);}
}
loadData();
loadUsers();
    ...
    if(key!=='limit' && key!=='offset' && params[key]){
      ...
      if(dateCols.includes(baseKey)){
        results=results.filter(item=>{
          ...
          const serial=typeof item[baseKey]==='number'?item[baseKey]:convertYYYYMMDDToExcelSerial(item[baseKey].toString());
          if(serial===null) return false;
          const startSerial=params[`${baseKey}_start`]?convertYYYYMMDDToExcelSerial(params[`${baseKey}_start`]):null;
          const endSerial=params[`${baseKey}_end`]?convertYYYYMMDDToExcelSerial(params[`${baseKey}_end`]):null;
          let ok=true;
          if(startSerial!==null) ok=ok&&(serial>=startSerial);
          if(endSerial!==null) ok=ok&&(serial<=endSerial);
          return ok;
        });
